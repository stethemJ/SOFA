...
from forward_model.tools.Convolution import beam_power_coefficients, antenna_temperature, sky_coefficients
...
hp.mollview(hp.ud_grade(raw_map, NSIDE), sub=(2, 1, 1), fig=fig1,
            title="Raw spectral-index sky (Galactic frame), f=100 MHz", unit="K")
...
analyze = Analyze(basis, HEIGHT, observer, l_max=L_MAX, nside=NSIDE)
...
ax.set_title(f"Realistic antenna temperature vs frequency (MRO, spectral-index sky, h={HEIGHT} m)")
ax.grid(True)
