...
Ylm linear combinations (10+ terms each) as 3D surfaces, radius = |value|,
colored by the signed value with plasma.
"""
import numpy as np
import matplotlib.pyplot as plt
from matplotlib import cm, colors

from forward_model.tools.ShBasis import ShBasis
from forward_model.tools.config import PROJECT_ROOT, OUTPUT_DIR
...
for ax, (title, basis) in zip(axes.flat, panels):
    values = basis.evaluate(theta_grid, phi_grid)
    r = np.abs(values)
    ...
    fig.colorbar(mappable, ax=ax, shrink=0.6, pad=0.05, label="Ylm value")

fig.tight_layout()

save_dir = OUTPUT_DIR / "tests"
save_dir.mkdir(parents=True, exist_ok=True)
save_path = save_dir / "SH_visual.png"
