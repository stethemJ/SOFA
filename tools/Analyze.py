...
class Analyze:
    ...
    def __init__(self, basis: VshBasis, height: float, observer: Observer,
                 sky_model: str = "2008", l_max: int = 40, nside: int = 128):
        self.basis = basis
        self.height = height
        self.observer = observer
        self.sky = Sky(model=sky_model)
        self.l_max = l_max
        self.nside = nside

    def antenna_temperature(self, frequency_mhz: float) -> float:
        sky_map = self.sky.generate(frequency_mhz)
    ...
    def spectrum(self, freqs_mhz: npt.ArrayLike) -> npt.NDArray[np.float64]:
        ...
        sky_maps = np.atleast_2d(self.sky.generate(freqs_mhz))
